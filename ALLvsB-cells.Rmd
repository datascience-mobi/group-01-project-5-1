...
The identification of recurrent genetic changes can help for refined individual disease prognosis and management. In our project we focus on the methylation status of promotors in ALL related genes. Based on a publication of Roman-Gomez we validate the methylation status of the 15 mentioned genes and further search for promoters and affiliated genes relevant for ALL. The data opens the possibility for new potential biomarkers for ALL risk prognosis and therapy.
...
Upon literature reseach we defined 14 promoters for the genes (CDH1, CDKN2A, CDKN2B, CDKN1C, KLK10, DKK3, CDH13, PYCARD, DAPK1, PARKIN, PTEN, p73, APAF1, LATS1) as "relevant" for our tumour type (Roman-Gomez *et al.*, 2004). These are from now on referred to as selected promotors/genes. In order to find an upper threshold we took several factors into account. First we checked how many of the selected promoters were removed for different thresholds. Even with a threshold of 99% of our coverage 3 promoters for genes (p73, APAF1, LATS1) always only had NA for the coverage values (any value outside of thresholds = NA). 
Next we checked how many of the remaining regions were removed while still keeping as many selected promoters additionally reducing the coverage as much as needed to avoid PCR mediated errors.
Finally we agreed on a lower threshold of 1.5 % (28) and an upper threshold of 98.5 % (7753) of the coverage values since only 10 % of the data and 2 additional selected promoter regions (p57, PTEN) were rejected. It formed the best compromise between the aforementioned factors.
Additionally we defined the extra variable ALLpromotorCov1 which is then used for the following analyses. Using ALLpromotorCov1 and its origin ALLpromotorCov we can later calculate the amount of data lost during the next steps of data cleaning.
...
Any remaining NAs are now replaced by the patient group's mean Beta-value in order to completely remove NAs in the Beta-values from our dataset. We chose the mean instead of a random number to ensure that we don't alter the result of the t-test in downstream analysis which uses the mean and standard deviation. Therefore the mean stays the same. The standard deviation does change but it seemed like a better option compared to a random number or complete deletion of said promotor.
```
...
First the actual PCA is executed. We used the transposed matrix in order to obtain PCs which are linked to the patients instead of the promoters themselves. 
```{r Performing PCA}
library(ggplot2)
...
In order to ascertain the absence of batch effects we used two different statistical tests: the Wilcoxon test and the Kruskal-Wallis test. We reviewed every single selected PC for the respective batch effect. Using the Wilcoxon test we checked the tissue type and disease status for batch effects.
...
With the Kruskal-Wallis test on the other hand we tested if biomaterial provider or first submission date caused any batch effects. Additionally we converted both investigated columns to factors in order to achieve finite values preventing computational errors.
...
When looking at the levelplot there is no p-value that is below 0.05 except for the disease row for PC1. This proves that the PC1 does show a significant difference between healthy and diseased patients which is exactly what we would want from the first principal component.


**Selection of segregating PC**

Next we need to choose a PC for later analysis which segregates the two patient groups. We achieved this through plotting the patients variance against the PCs. If a PC can sort the patient groups it is selected for downstream analysis.
```{r Creating levelplot of PCs}
library(lattice)
pca_x_abs <- abs(pca$x[,c(1:3)])
levelplot(t(pca_x_abs), xlab = "PCs", ylab = "Patients", main = "Plot PCA Betrag")
```

Using the information of the last two levelplots the first principal component is selected for further analysis since it represents a good seperation of the two patient groups while not showing any significant batch affects


**Selection of relevant promoters**

Next the most impotant promoters for that PC need to be selected. We achieved this by sorting the loading and variance of the promoters related to the first principal component. 
As a first test we sorted the absolute values of the loadings of the promoters and again chose a cutoff at the spot where we found an elbow.
```{r Plotting loading}
pcaRotAbs <- abs(pca$rotation[,1])
pcaRotAbssort <- sort(pcaRotAbs, decreasing = TRUE)
plot(pcaRotAbssort, type = "l", main = "Plot der PC1 Loading", xlab = "Genes", ylim = c(0,0.06), xlim = c(0,1000))
```

However we found the elbow at approximately 50 promoters which is a very low amount of data to use. Therefore we tried again using the variance of each promoter since this value also related to the differentially methylated regions. We again sorted the values in a decreasing order and plotted them.
```{r Plotting variance}
pcaRotVar <- apply(pca$rotation, 1, var)
pcaRotVarsort <- sort(pcaRotVar, decreasing = TRUE)
plot(pcaRotVarsort, type = "l", main = "Plot der Genvarianz", xlab = "Genes", ylim = c(0,0.0005), xlim = c(0,2000))
```

The elbow for the variance was located at 700 promoters which represents a much wider base to support downstream analysis. Therefore we continued with these 700 relevant promoters and filtered them.
First we ordered the variances in order to obtain the position of the relevant promoters. Then the promoters above the threshold of 700 were removed and the M-values were extracted to match the remaining 700 promoters.
...
In order to incorporate the selected genes from the previous literature research the affiliated promoters are added to the 700 filtered promoters. This leads to a dataframe containing the selected as well as the filtered promoters ready for further analysis.
...
As an additional step we performed k-means clustering to ensure the segregation of the patients into their respective groups. We checked the seperation through a silhouette plot.
...
The silhouette plot and k-means clustering data show a good seperation of our patients into their respective health status. Now our data is ready to be analysed for differentially methylated regions via t-test.



###Step3 - Testing for differentially methylated regions (DMR)

In order to find the promotors which are significantly differentially methylated the 709 filtered promoters are checked via t-test. This is the best method to find recurrent differences since it uses the mean of each patient group and therefore a differential methylation has to occur in most patients to be significant. If there is a DMR in just one patient i will not be recognized as recurrent and will therefore not be significant since the mean does not account for single outliers. 

**Preparing for the t-test**
First the sample originating from the tonsil of the control group patients are removed from the analysis since none grouped with the sample derived from the bone marrow. Additionally all samples for the diseased group originated from the bone marrow which would lead to false results when comparing to a different tissue type. Then the M-values are splitted into their respective health groups healthy and diseased to make the t-test possible.
...
Next a vector is created which will contain the p-values of the t-test.
...
Now the actual t-test is performed and the resulting p-values are stored in the vector t_test for later use.
...
In order to account for the statistical problem of multiple comparisons the p-values are adjusted using the Holm-Bonferroni method. The corrected values are then ordered and sorted in an increasing manner.
...
Now the 20 most significant promoters are selected for in depth analysis e.g. its function and therefore the advantage for the cell. Additionally the Beta- and M-values of these selected promoters are extracted for later plotting.
...
For the following logistical regression the significant promoters (\alpha <= 5%) were calculated. Again a dataframe is created containing the corresponding Beta- and M-values.
...
Next a levelplot is created to visualize the Beta-values of the 20 most significant promoters.
...
The same is done for the M-values. Both matrices are prepared beforehand to ensure no computational errors during plotting.
...
levelplot(ALLMvalueRemain20_plot, xlab = "Genes", ylab = "Patients", main = "Levelplot M-values of 20 remaining Genes")
```


**Plotting Beta- and M-values for the significant (\alpha <= 5%) promoters**

The same steps as above are repeated for the significant promoters.
...
levelplot(ALLBetaRemain_threshold_plot, xlab = "Genes", ylab = "Patients", main = "Levelplot Beta-values of remaining Genes")
```
...
levelplot(ALLMvalueRemain_threshold_plot, xlab = "Genes", ylab = "Patients", main = "Levelplot M-values of remaining Genes")

```


**Plotting M-values of selected genes**

Lastly a levelplot depicting the M-values of the selected genes/promoters from the literature research is created in order to analyse them separately.
```{r Plotting M-value levels of selected genes}
ALLMGen_plot <- ALLMGen
colnames(ALLMGen_plot) <- c(1:ncol(ALLMGen))
levelplot(as.matrix(t(ALLMGen_plot)), xlab = "Patients", ylab = "Genes", main = "Levelplot M-values of selected Genes")
```
...
As a final step we wanted to create a model which could predict the cancer status of a patient using logistical regression. We chose logistical regression over linear regression since the latter is much less accurate when it comes to binary events such as methylation. Therefore a logistically regressed model would achieve better results predicting a patients disease status using methylation data. 
Using the previously defined 99 significant genes we created a dataframe which also define the health status of the patients data used to create the model. Diseased patients were defined as "1" and healthy patients as "0". Therefore upon prediction a value higher than 0.5 would indicate a cancerous state versus a healthy state if the predicted value is lower than 0.5.
Here the dataframe is prepared and the additional column with the health status is added.
...
Next a matrix is created in which the predicted values are stored. Then each of the significant genes is used for logistical regression and prediction of health status. Once all prediction values are stored in the matrix the first column used to create the matrix is removed.
...
Finally the predicted values are plotted via a levelplot to check for the accuracy of our model.
```{r Plotting predictions}
levelplot(t(LGpred), xlab = "Genes", ylab = "Patients", main = "Levelplot of logistical regression prediction")
```
